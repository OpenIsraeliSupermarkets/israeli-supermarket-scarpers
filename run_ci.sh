...
#   docs.yml            → Step: Docs
#   python-publish.yml  → Step: Package (check-packages only)
#
# Skipped (require GitHub Actions infra):
#   codeql.yml, user-validation.yml, docker-publish.yml
#   python-publish.yml deploy (PyPI upload)
#
# Usage: ./run_ci.sh [--no-cache] [--skip-tests] [--skip-lint] [--skip-docs] [--skip-package] [N]
...
#   --skip-package  skip the package check stage
...
if $RUN_DOCS; then
    ...
    # Wipe generated API stubs first: sphinx-apidoc --force overwrites but does
    # not remove orphans (e.g. deleted modules), which then fail autodoc under -W.
...
# ---------------------------------------------------------------------------
# Step 4: Package  (mirrors python-publish.yml check-packages)
# ---------------------------------------------------------------------------
if $RUN_PACKAGE; then
    print_step "Package (python-publish.yml)"

    if python -m pip install --quiet --upgrade pip && \
       python -m pip install --quiet build setuptools && \
       python scripts/check_setup_packages.py && \
       python -m build; then
        pass "Package"
    else
        fail "Package"
