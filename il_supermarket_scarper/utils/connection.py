...
def download_connection_retry():
    ...
    def wrapper(func):
        ...
        @retry(exceptions=(URLError,RemoteDisconnected,ConnectionResetError,
                                                socket.gaierror,socket.timeout),
                                                tries=5,delay=2,backoff=2,
                                                max_delay=30,logger=Logger)
...
def disable_when_outside_israel(function):
    ...
    def _decorator():
        Logger.warning(f"test {function.__name__ } has been disabled!"
