...
def make_test_case(init_scraper_function):
    ...
    class TestScapers(unittest.TestCase):
        ...
        def _clean_scarpe_delete(self,init_scraper_function,
                                    dump_path="temp",
                                    limit=None,file_type=None):
        ...
        def test_scrape_one(self):
            ...
            self._clean_scarpe_delete(init_scraper_function,self._get_temp_folder(),limit=1)

        def test_scrape_ten(self):
            """ scrape ten file and make sure they exists """
            self._clean_scarpe_delete(init_scraper_function,self._get_temp_folder(),limit=10)

        def test_scrape_promo(self):
            """ scrape one promo file and make sure it exists """
            self._clean_scarpe_delete(init_scraper_function,self._get_temp_folder(),
                                            limit=1,file_type=FileTypesFilters.only_promo())

        def test_scrape_store(self):
            """ scrape one store file and make sure it exists """
            self._clean_scarpe_delete(init_scraper_function,self._get_temp_folder(),
                                            limit=1,file_type=FileTypesFilters.only_store())

        def test_scrape_price(self):
            """ scrape one price file and make sure it exists """
            self._clean_scarpe_delete(init_scraper_function,self._get_temp_folder(),
                                            limit=1,file_type=FileTypesFilters.only_price())
