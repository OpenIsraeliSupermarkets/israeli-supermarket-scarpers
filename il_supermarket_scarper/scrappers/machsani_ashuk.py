...
class MahsaniAShukNewSource(ApiWebEngine):
    ...
    async def get_request_url(self, files_types=None, store_id=None, when_date=None):
        ...
        for chain_id in self.get_chain_id():
            branches = self.get_api_data("/webapi/api/getbranches", {"edi": chain_id})
    ...
    async def extract_task_from_entry(self, all_trs):
        ...
        for entry in all_trs:
            try:
                ...
                if not file_name:
                    continue
                chain_id = self.get_chain_id()[0]
                download_url = f"{self.url.rstrip('/')}/webapi/{chain_id}/{file_name}"
    ...
    def dedupe_api_entries(self, all_entries):
        """Keep first occurrence of each file name."""
        seen = set()
        out = []
    ...
    def apply_filter_by_type(self, entries, files_types=None):
        ...
        return [e for e in entries if e.get("fileType", "").lower() in wanted_types]
